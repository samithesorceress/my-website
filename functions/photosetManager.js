var photosetManager = {
	actions: {
		...
		delete: function (e) {
			...
			if (selected.length) {
				dialogBox.add("confirmation", "Are you sure you want to delete these items? This action cannot be undone.", "main",photosetManager.confirmDelete, selected);
			}
		}
	},
	confirmDelete: function (res, args) {
		console.log("confirming delete..");
		var api_endpoint = "deletePhotoset",
			api_params = [];
			ids = "";
		...
		if (res === true) {
			...
			util.xhrFetch(api_endpoint, api_params, photosetManager.updateGrid);
		}
	},
...
if (actions) {
	...
	for (var i = 0; i < action_btns.length; i += 1) {
		...
		switch(btn.dataset.action) {
			case "edit":
				btn.addEventListener("click", photosetManager.actions.edit);
				break;
			case "delete":
				btn.addEventListener("click", photosetManager.actions.delete);
				break;
		}
	}
}