...
if (valExists("success", $about_res)) {
	...
	echo card(false, false, newFormField("profile", "Profile Photo", "media_browser", 1, $profile_photo));
	
	$bio_text = false;
	if (valExists("bio", $about_data)) {
		$bio_text = urldecode($about_data["bio"]);
	}
	echo card(false, false, newFormField("bio", "Bio", "textarea", $bio_text));

	$links = false;
	if (valExists("links", $about_data)) {
		$links = $about_data["links"];
	}
	echo card(false, false, newFormField("links", "Links", "links", $links));
