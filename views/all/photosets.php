...
<section id="view_all" class="card">
	<div class='card_contents'>
		<ul id='view_all_list' data-type="photoset">
		<?php
			$photosets_api = "listPhotosets?rows=5&order_by=id&order_dir=DESC";
			$photosets_results = xhrFetch($photosets_api);
			if (valExists("success", $photosets_results)) {
				$photosets = $photosets_results["data"];
				if (valExists("id", $photosets)) { // only one result was returned
					$photosets = [$photosets];
				}
				if ($photosets) {
					foreach ($photosets as $photoset) {
						echo "<li id='list_item_" . $photoset["id"] . "' class='wide' data-key='" . $photoset["id"] . "' oncontextmenu='selectItems.toggle(event)'>";
							...
							echo "</button>";
							echo "<a href='" . $admin_root . "edit/photoset/" . $photoset["id"] . "'>";
